...
class GeneralParser implements ParserInterface
{
    ...
    public function parseLink()
    {
        ...
        if (!strncmp($link->getContentType(), 'text/', strlen('text/'))) {
            ...
            $link->setTitle($htmlData['title'])
                ...
                ->setAuthor($htmlData['author'])
                ->setPublishDate($htmlData['publishdate'])
                ->setKeywords($htmlData['keywords']);
    ...
    protected function parseHtml($html)
    {
        ...
        foreach ($doc->getElementsByTagName('meta') as $meta) {
            ...
            } elseif ($meta->getAttribute('property') === 'image') {
                $data['image'] = $meta->getAttribute('value');
            } elseif($meta->getAttribute('name') === 'keywords') {
                $data['keywords'] = $meta->getAttribute('content');
            } elseif(preg_match("/publish/i", $meta->getAttribute('name'))) {
                if(!preg_match("/unix/i", $meta->getAttribute('name'))){
                    $data['publishdate'] = date('Y-m-d H:i', strtotime($meta->getAttribute('content')));
                }
            } elseif(preg_match("/publish/i", $meta->getAttribute('property'))) {
                $data['publishdate'] = date('Y-m-d H:i', strtotime($meta->getAttribute('content')));
            } elseif(preg_match("/author/i", $meta->getAttribute('name'))) {
